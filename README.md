...
│   ├── calibrate_dual.py             # per-channel multi-point DC calibration
...
**Done:** hardware bring-up · local capture + host transfer · per-channel calibration · front-end system ID · Method B digital compensation (Python + MATLAB) · noise/performance validation · anti-alias filter designed, built, and characterized (kept as a spare) · burst-quality MATLAB pipeline · full build guide (Markdown + PDF).

This documentation set is complete; the repository is the canonical, up-to-date record of the build.
